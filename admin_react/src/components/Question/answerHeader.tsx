...
export const AnswerHeader:React.FC<{header:Partial<Question>,author:Partial<User>}>=({author,header})=>{
    ...
    return <Flexbox h="15vh" direction="row" just="space-between" align="flex-start">
            ...
            <div><span style={{marginRight:'5px'}}>Video:</span><Span w="40vw" color="black">{header.course?.title}</Span></div>
                <div><span style={{marginRight:'5px'}}>Question:</span><Span color="black" w="40vw">{header.content}</Span></div>
            </Flexbox>
            <Avatar size={60} src={author.avatar}/>
    </Flexbox>
}