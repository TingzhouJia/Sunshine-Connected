...
export class AuditRepository extends BaseRepository<Audit>{
    constructor(@InjectModel(Audit) private readonly _model:ReturnModelType<typeof Audit>){
        super(_model)
    }
    ...
    async getMyAudit(pagination:PaginationParams<Audit>,id:string){
        return super.paginator({...pagination,query:{auditor_id:id}},'',{populates:[{path:'progress',},{path:'object'}]})
    }
}